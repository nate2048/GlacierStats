...
def nearest_neighbor_search(radius, num_points, loc, data2):
    ...
    indicies = torch.arange(len(data2)).cuda()
    ...
    smallest = torch.full((num_points, 2), float('nan')).cuda()
    index_list = torch.full((num_points,), float('nan')).cuda()
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        sgs = skrige_sgs_parallel(Pred_grid_xy, torch_data, k, vario, rad, num_gpus)
